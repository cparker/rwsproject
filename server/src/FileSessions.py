...
class ManagedSessionInterface(SessionInterface):
    ...
    def save_session(self, app, session, response):
        ...
        cookie_exp = datetime.datetime.now() + self.get_expiration_time(app, session)
        realTime = time.mktime(cookie_exp.timetuple())
        response.set_cookie(app.session_cookie_name,
                            '%s!%s' % (session.sid, session.hmac_digest),
                            expires=realTime, httponly=True, domain=domain)